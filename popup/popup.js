...
function render() {
  ...
  const cards   = Object.values(workspaces).sort((a, b) => a.createdAt - b.createdAt);
...
function showConfirm(id) {
  ...
  if (!ws) return;
  pendingDeleteId = id;
  document.getElementById("confirm-text").textContent = `Smazat workspace "${ws.name}"?`;
...
function showModal(wsId = null) {
  ...
  document.getElementById("modal-title").textContent   = ws ? "Přejmenovat workspace" : "Nový workspace";
  document.getElementById("btn-save").textContent      = ws ? "Uložit" : "Vytvořit";
